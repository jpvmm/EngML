...
def model_training(
    ...
    region: str = REGION,
    serving_container: str = "us-east1-docker.pkg.dev/qacomp/custom-predictor-repo/custom-predictor:latest"
):
    dataset = read_and_process_data(input_path = input_path)
    train_task = train_model(train_set=dataset.outputs['train'], test_set=dataset.outputs['test'])
